...
-(void)initUI {    
    self.thumbImageView.image = self.currentNew.thumbnail;
    self.headerLabel.text = self.currentNew.title;
    
    int height = 0;
    
    if (!self.photosCountLabel) {
        self.photosCountLabel = [[UILabel alloc] initWithFrame:CGRectMake(15, 10, 300, 0)];
    ...
    self.photosCountLabel.text = [NSString stringWithFormat:@"%d фотографий", self.currentNew.photosCount];
    ...
    int row = 0, column = 0;
    for (int i = 0; i < self.currentNew.photoURLs.count; i++) {        
        ...
        [button setImageWithURL:[self.currentNew.photoURLs objectAtIndex:i] placeholderImage:kPLACEHOLDER_IMAGE andScaleTo:CGSizeMake(64, 64)];
//        UIImage *image;
